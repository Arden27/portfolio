export default function ProjectBox({ title, placeSelfEnd, translateDirection }){
    return(
    <div className={`group md:relative md:aspect-square max-sm:w-full max-sm:flex-shrink-0 max-sm:snap-center h-full ${placeSelfEnd ? 'place-self-end' : ''} md:hover:cursor-pointer`}>
      ...
      <div className={`md:group-hover:${translateDirection} h-full md:absolute md:inset-0 rounded-2xl border-2 border-black bg-green-300 md:transition-transform md:duration-700 md:ease-in-out md:flex md:items-center md:justify-center`}>
